...
class AdaptiveRouter:
    ...
    async def _llm_classify(self, task: str, registry: ToolRegistry) -> str:
        ...
        prompt = (
            ...
            "- clarify：任务过于模糊、缺少必要对象（如没说查哪位客户、统计什么），无法执行。\n"
            "关键区分：任务目标和对象都已明确、只是宿主现有工具做不到时，不要判 clarify，\n"
            "应判 react（先用现有工具取数，再由系统补充能力），"
            "并在 missing_capability 写明缺失能力；\n"
            "只有“任务本身说不清要做什么/对谁做”时才判 clarify。\n"
