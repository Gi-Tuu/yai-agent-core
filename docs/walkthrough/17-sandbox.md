...
## 4. 未来代码工具的生命周期（不在本契约内）

模块 docstring 末尾特意划清边界：代码工具的"默认 48 小时 TTL、被多次调用刷新 TTL、
后台可置为永久保留"等，属于未来的**代码工具注册表**层，不属于这个执行契约。路线见
`ROADMAP.md`。换句话说，本文件只回答"怎么安全执行一次"，不回答"生成的工具活多久"。
