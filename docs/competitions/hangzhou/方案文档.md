...
| 冻结 commit | `81729f9d39d1f2f0db279265ab6cc5d387a3921f`（线上 /health 实时返回该值） |
...
- **在线可验收**：`GET https://yai-agent-core.onrender.com/health` 实时返回 `{"status":"ok","commit":"81729f9…"}`；`GET /v1/tools` 列出当前工具；`POST /v1/agent/run`（请求体 `{"task": "..."}`）为真实能力端点。
- **测试**：**159 项离线测试全绿**（假模型 / httpx MockTransport / 注入时钟，不依赖网络与 API Key），覆盖路由、循环、上下文压缩、记忆、保留策略、MCP 桥、OpenAPI 桥、限流、API，以及 host_e 业务层、零改造嵌入与读写权限分级。
- **CI**：GitHub Actions 四矩阵（Ubuntu Python 3.11/3.12/3.13 + Windows 3.13）跑 ruff + pytest，最近一次 run 成功。
- **可复现**：`uv sync --extra dev --extra llm --extra server --extra mcp --extra openapi` → `uv run pytest`（159 项全绿，命令与 CI 一致）→ `uv run python scripts/smoke_test.py`（一个内核冒烟跑通三个宿主）；`docker compose up --build` 容器化（容器内 8000）；PaaS 部署件为 `render.yaml` + `Dockerfile`。
...
- **泛用性实证**：仓库内置 5 个宿主示例——办公笔记（host_a，3 个本地工具）、数据应用（host_b）、AI 陪伴（host_c，5 个工具：档案/回忆/记忆/心情/关怀建议）、本地 MCP 演示（host_d）、销售 CRM（host_e，12 个业务工具）。**同一份内核，不改一行内核代码，换 capabilities 即换岗位**。
- **接入成本低（host_e 实证）**：host_e 首先是一个不依赖 YAI 也能完整运行的销售 CRM（菜单界面、JSON 持久化、12 个业务方法，业务层零 yai_core 导入）；嵌入时**同一个 `SalesCrm` 业务对象**交给 `AgentCore.auto`，新增代码仅一个约 30 行的入口文件——8 个读工具自动放行、4 个写工具（写跟进/建待办等）执行前在终端 `[y/N]` 授权，多步任务自动规划并产出销售日报。宿主工具就是普通 Python 函数/业务对象的公开方法，类型注解 + docstring 自动转 JSON Schema（与 MCP 工具同构），无需学习新框架概念。
...
- **主场景（host_a 办公提效，在线）**：3 条虚构笔记 + `list_notes(tag)` / `search_notes(keyword)` / `count_notes()` 三个工具，演示"统计笔记数 → 按关键词检索 → 汇总"端到端闭环，画面带事件流。
- **核心场景（host_e 销售 CRM，本地）**：先演示独立菜单软件（无 AI 完整可用、业务层零内核导入），再演示同一 `SalesCrm` 对象零改造嵌入后的数字员工——一句"查超期客户 → 写跟进 → 建待办 → 出日报"，plan 拆 4 步、读操作自动执行、两次写操作先后弹授权确认、日报中可见状态真实变化（刚跟进的客户立即移出超期名单）。
- **换岗位（host_c，压缩段）**：同一内核切到陪伴宿主，工具集与策略随之变化，证明泛化。
...
git checkout 81729f9d39d1f2f0db279265ab6cc5d387a3921f
uv sync --extra dev --extra llm --extra server --extra mcp --extra openapi
uv run pytest                      # 159 项离线测试（命令与 CI 一致）
...
| 维度 | 最强证据 |
|---|---|
| 技术可行性 | 线上 API + 159 测试 + CI 四矩阵 + Docker/Render 部署 |
| 市场可行性 | 5 宿主泛化实证（含 host_e 普通软件零改造嵌入）+ AMBRACE 回流路线 |
