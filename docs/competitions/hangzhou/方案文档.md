...
- **测试**：**141 项离线测试全绿**（假模型 / httpx MockTransport / 注入时钟，不依赖网络与 API Key），覆盖路由、循环、上下文压缩、记忆、保留策略、MCP 桥、OpenAPI 桥、限流、API，以及 host_e 业务层、零改造嵌入与读写权限分级。
- **CI**：GitHub Actions 四矩阵（Ubuntu Python 3.11/3.12/3.13 + Windows 3.13）跑 ruff + pytest，最近一次 run 成功。
- **可复现**：`uv sync --extra dev --extra llm --extra server --extra mcp --extra openapi` → `uv run pytest`（141 项全绿，命令与 CI 一致）→ `uv run python scripts/smoke_test.py`（一个内核冒烟跑通三个宿主）；`docker compose up --build` 容器化（容器内 8000）；PaaS 部署件为 `render.yaml` + `Dockerfile`。
...
uv run pytest                      # 141 项离线测试（命令与 CI 一致）
...
| 维度 | 最强证据 |
|---|---|
| 技术可行性 | 线上 API + 141 测试 + CI 四矩阵 + Docker/Render 部署 |
