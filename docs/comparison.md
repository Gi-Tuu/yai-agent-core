...
| 工具发现匹配 | **双通道：词法（零依赖、中文 bigram、同义词）+ 可选 embedding 语义**，标准库余弦、两层摘要、margin 防误召回；不配置 embedder 自动降级纯词法 | 通常靠开发者手工绑定或全量塞 prompt，无内置语义检索 | 无内置 | 平台内固定工具集，按名称选择 | 平台插件市场 |
...
| 路由自校准 | **从执行反馈学习的上下文老虎机**（按任务特征分桶的 Beta/Thompson，规则先验冷启动），同类任务越用越准；离线 benchmark 可复现（末段 89.6% vs 规则 73.3%），默认关闭 | 无内置，需自行实现评估/微调回路 | 无内置 | 无内置 | 无内置 |
...
| 成熟度 | 单人新项目，388 项测试、7 个示例宿主 | 成熟大生态 | 成熟社区 | 成熟平台（156K+ Star） | 商业产品 |
...
4. **路由会从反馈里自校准（self-adaptive 坐实）。** 关键词/LLM 路由是开环的，判一次就结束；YAI 可选地在任务结束后从事件流抽取成败，按任务特征分桶累计 Beta(α,β) 后验，用 Thompson Sampling 为同类任务调整策略，冷启动确定性跟随规则、有真实反馈后才采样。纯标准库、零依赖、离线可复现：内置 benchmark 五线（含"无先验""无上下文"消融）显示末段成功率 89.6% vs 规则 73.3%。学习默认关闭、硬规则区域不表态，学习算法本身也是第七个 SPI（`RouteSelector`）可替换。
5. **内核零硬依赖 + SPI 八契约。** 内核本体 `dependencies = []`，模型/通道/记忆/权限/发现源/沙箱/路由学习/文本嵌入全部可替换；这让内核可以进入移动端、桌面端、受限网络等装不下框架全家桶的环境。
6. **工具发现是"双通道"而非关键词。** 关键词匹配召不回"出门该怎么穿、要不要带外套"这类没有"天气"字样的缺口；YAI 的 `SemanticCatalog` 用词法（零依赖、永远可用）+ 可选 embedding 语义两条独立通道召回，余弦用标准库 `math` 计算（不引 numpy 进内核），并用"绝对门槛 + 多候选 top1 边际"避免在一堆相似候选里误召回；嵌入后端可插拔（本地 bge-m3 离线免费，或 OpenAI 兼容云端），不配置或报错自动降级纯词法。阈值用真实 bge-m3 校准而非拍脑袋。
