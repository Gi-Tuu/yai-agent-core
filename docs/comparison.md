...
> 最后更新：2026-09-21。本文只写可验证事实：YAI 一侧以本仓库代码/配置为准，同类方案以其官方文档表述为准；不做贬低式比较，文末给出"什么场景该选谁"。
...
| 成熟度 | 单人新项目，197 项测试、6 个示例宿主 | 成熟大生态 | 成熟社区 | 成熟平台（156K+ Star） | 商业产品 |
...
4. **内核零硬依赖 + SPI 六契约。** 内核本体 `dependencies = []`，模型/通道/记忆/权限/发现源/沙箱全部可替换；这让内核可以进入移动端、桌面端、受限网络等装不下框架全家桶的环境。
...
- YAI 事实：本仓库 `pyproject.toml`（dependencies/extras）、`src/yai_core/`、`README.md`、`ROADMAP.md`（2026-09-20 复核）。
