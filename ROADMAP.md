...
- **两层工具目录**：注册表提供 `catalog()`（名称 + 一句话摘要）与 `schemas_for(names)`（按需返回完整 schema），为大规模工具集的上下文瘦身打好 API。
- **每轮反思**：工具失败后把失败原因回灌模型，引导其改用现有工具或如实说明能力缺口，不再反复撞同一失败调用。
...
- **代码生成工具沙箱契约（已定义，未实现）**：新增第六个 SPI `ToolSandbox` / `SandboxResult`，内核不内置任何代码执行器；见下文"代码生成工具"。
- 网页工作台渲染 `capability_missing` / `tool_composed` 事件。
...
- **代码生成工具（Code Tool）**：在组合工具之上，允许模型在沙箱里生成并执行新代码，产生宿主原本没有的能力。`ToolSandbox` SPI 已在 v0.7 定义，内核不内置沙箱，由宿主按部署形态提供（子进程 + RestrictedPython / 一次性容器 / 不实现则降级）。
  - 授权：除"无需审批"档外，创建与首次执行代码工具都必须经 `PermissionPolicy` 取得用户授权；
  - 生命周期：代码工具默认临时保留 48h，期间被多次调用则刷新 TTL；宿主可在后台把某个工具置为永久保留；
  - 隔离：默认无网络、无敏感文件访问，强制超时与资源上限；
